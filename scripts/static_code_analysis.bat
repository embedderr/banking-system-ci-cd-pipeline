...
cppcheck --xml --enable=all --std=c99 %includes% --platform=%platfrm% %files% --output-file=reports\static_file.xml

echo [2/3] Running MISRA Check...
cppcheck --xml --enable=all --std=c99 %includes% --platform=%platfrm% --addon=./static_code_analysis/misra-config.json %files% --output-file=reports\MISRA_file.xml
